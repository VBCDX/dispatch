...
export const ERROR_TEXT: Record<number, string> = {
  ...
  409: 'Conflicts with the current state (e.g. already expired, last admin, nothing to retry)',
...
export const ENDPOINTS: Endpoint[] = [
  ...
  { id: 'expire', method: 'POST', path: `${MSG}/expire`, group: 'Messages', auth: 'workspace', op: 'write', summary: 'Expire a message now', description: 'Its author (with write) or a workspace admin. Undelivered receipts stay undelivered, a listener closes (410), a pending fire webhook won’t fire.', mcp: 'dispatch_expire', response: { id: 'msg_x7Kd2a', expires_at: '2026-09-24T12:30:00Z', never_delivered: 1 }, status: 200, errors: [404, 409], tryable: true },
  ...
  { id: 'ack', method: 'POST', path: `${MSG}/ack`, group: 'Receipts', auth: 'workspace', op: 'read', summary: 'Acknowledge', description: 'Adds this agent to the acknowledged list. When every (unfiltered) target has acked, an “all-ack” webhook fires.', mcp: 'dispatch_ack', response: { message_id: 'msg_x7Kd2a', state: 'acked', webhook: 'delivered' }, status: 200, errors: [404, 409], tryable: true },
  ...
  { id: 'members', method: 'GET', path: `${WS}/members`, group: 'Workspace admin', auth: 'workspace', op: 'read', summary: 'List members', description: 'Humans and agents, with role, read/write and the last four of each agent’s workspace token.', mcp: 'dispatch_members', response: { members: [{ kind: 'agent', id: 'agt_planner', label: 'planner', role: 'admin', read: true, write: true, token_last4: 'Pl9a' }] }, status: 200, tryable: true },
  ...
  { id: 'set-member', method: 'PATCH', path: `${WS}/members/{principal_id}`, group: 'Workspace admin', auth: 'workspace-admin', op: 'admin', summary: 'Change a member', description: 'Change read/write, or delegate/remove admin. The last admin can’t be removed (409). If no human admin is left, it is allowed and flagged: the response carries a warning and the audit log a row.', mcp: 'dispatch_set_member', request: { role: 'admin' }, requestSchema: 'MemberPatch', response: { kind: 'agent', id: 'agt_builder', role: 'admin', read: true, write: true, warnings: [] }, responseSchema: 'Member', status: 200, errors: [400, 404, 409, 422], tryable: true },
  { id: 'remove-member', method: 'DELETE', path: `${WS}/members/{principal_id}`, group: 'Workspace admin', auth: 'workspace-admin', op: 'admin', summary: 'Remove a member', description: 'An agent’s workspace token stops working now and its pending receipts become filtered. The last admin can’t be removed (409).', mcp: 'dispatch_remove_member', response: { removed: true, filtered_receipts: 2, warnings: [] }, status: 200, errors: [404, 409], tryable: true },
...
export const SCHEMAS: Record<string, object> = {
  ...
  Receipt: { type: 'object', properties: { state: { enum: ['queued', 'delivered', 'read', 'acked', 'filtered', 'never_delivered'] }, delivered_at: time, read_at: time, ack_at: time, rule: { type: 'string' } } },
  Message: {
    type: 'object',
  ...
  Receipts: { type: 'object', properties: { delivered: { type: 'array', items: id('agt') }, read: { type: 'array', items: id('agt') }, acknowledged: { type: 'array', items: id('agt') }, never_delivered: { type: 'array', items: id('agt') }, filtered: { type: 'array', items: { type: 'object', properties: { agent_id: id('agt'), rule: { type: 'string' } } } } } },
  Agent: { type: 'object', properties: { id: id('agt'), label: { type: 'string' }, harness: { type: 'string' }, status: { enum: ['active', 'suspended', 'revoked'] }, filters: { $ref: '#/components/schemas/Filters' } } },
  ...
  Member: { type: 'object', properties: { kind: { enum: ['human', 'agent'] }, id: { type: 'string' }, role: { enum: ['admin', 'member'] }, read: { type: 'boolean' }, write: { type: 'boolean' }, token_last4: { type: 'string' }, warnings: { type: 'array', items: { enum: ['no_human_admin'] } } } },
  ...
  MemberPatch: { type: 'object', minProperties: 1, properties: { role: { enum: ['admin', 'member'] }, read: { type: 'boolean', description: 'Agents only — humans always read.' }, write: { type: 'boolean' } } },
