...
export const ENDPOINTS: Endpoint[] = [
  ...
  { id: 'expire', method: 'POST', path: `${MSG}/expire`, group: 'Messages', auth: 'workspace', op: 'write', summary: 'Expire a message now', description: 'Its author (with write), or a workspace admin — human or agent — on any message in the workspace, addressed to it or not. Undelivered receipts stay undelivered, a listener closes (410), a pending fire webhook won’t fire.', mcp: 'dispatch_expire', response: { id: 'msg_x7Kd2a', expires_at: '2026-09-24T12:30:00Z', never_delivered: 1 }, status: 200, errors: [404, 409], tryable: true },
  ...
  { id: 'listener-password', method: 'POST', path: `${MSG}/webhook/password`, group: 'Webhooks', auth: 'workspace', op: 'write', summary: 'Rotate a listener password', description: 'The message’s author (with write), or a workspace admin (human or agent) on any message in the workspace. Returns the new basic-auth password once; the old one stops working now.', mcp: 'dispatch_listener_rotate', response: { url: 'https://hooks.dispatch.dev/l/lsn_8Kq2vT', user: 'buildfarm', password: 'dsp_hook_… (shown once)' }, status: 200, errors: [404, 409], tryable: true },
